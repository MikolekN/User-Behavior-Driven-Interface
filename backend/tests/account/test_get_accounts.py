...
def test_get_accounts_user_not_exist(client, test_user):
...
def test_get_accounts_not_exist(client, test_user):
    with patch('flask_login.utils._get_user', return_value=test_user), \
        ...
        response = client.get('/api/accounts/')
        assert_json_response(response, HTTPStatus.NOT_FOUND, "accountsNotExist")
...
def test_get_accounts_successful(client, test_user, test_accounts):
    with patch('flask_login.utils._get_user', return_value=test_user), \
        ...
        response = client.get('/api/accounts/')
        assert_json_response(response, HTTPStatus.OK, "accountsFetchSuccessful")
