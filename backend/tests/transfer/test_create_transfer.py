...
def test_create_transfer_missing_data(client, test_user):
    with patch('flask_login.utils._get_user', return_value=test_user):
        ...
        assert_json_response(response, HTTPStatus.BAD_REQUEST,
                             'missingFields;sender_account_number,recipient_account_number,title,amount')
...
def test_create_transfer_empty_field_data(client, test_user):
    with patch('flask_login.utils._get_user', return_value=test_user):
        ...
        assert_json_response(response, HTTPStatus.BAD_REQUEST,
                             'emptyFields;sender_account_number,recipient_account_number,title')
...
def test_create_transfer_unauthorised(client, test_user, test_unauthorised_account):
    with patch('flask_login.utils._get_user', return_value=test_user), \
            patch('accounts.account_repository.AccountRepository.find_by_account_number',
                  return_value=test_unauthorised_account):
        response = client.post('/api/transfer', json={
            "sender_account_number": "sender_account_number",
            "recipient_account_number": "recipient_account_number",
            "title": "title",
